...
export default function DashboardMember() {
  ...
  const cards = [
    { title: "Leads", count: stats.cabuysCount || 1234, icon: <Users size={55} strokeWidth={2.5} />, colorClass: "bg-blue-50" },
    { title: "Proyek", count: stats.projectCount || 56, icon: <Building2 size={55} strokeWidth={2.5} />, colorClass: "bg-blue-50" },
    { title: "Surve", count: stats.surveyCount || 342, icon: <ClipboardList size={55} strokeWidth={2.5} />, colorClass: "bg-blue-50" },
    { title: "Properti", count: stats.propertyCount || 289, icon: <Home size={55} strokeWidth={2.5} />, colorClass: "bg-blue-50" },
  ];
  ...
  return (
    <div className="min-h-[70vh]">
      <header className="mb-6">
        <h1 className="text-3xl font-extrabold text-black">Dashboard Member</h1>
        <p className="text-sm text-black mt-1">Selamat datang kembali! Berikut adalah ringkasan aktivitas Anda hari ini.</p>
      </header>
        ...
        {cards.map((c, i) => (
          <div key={i} className={`flex items-center justify-between p-5 rounded-lg shadow-sm ${c.colorClass}`}>
            <div className="flex items-center gap-4">
              ...
              <div>
                <div className="text-base text-gray-900">{c.title}</div>
