...
import RumahFormModal from "../../components/RumahFormModal"; // Pastikan path sesuai
import { 
  Plus, Edit, Trash2, MapPin, 
  Briefcase, Phone, Building2, 
  Home, Layers, MoreVertical 
} from "lucide-react";
...
export default function PropertiAdmin() {
  ...
  if (error) return (
    <div className="flex flex-col items-center justify-center h-64 text-red-500">
      ...
      <p>{error}</p>
      <button onClick={fetchProperti} className="mt-4 text-blue-600 underline">Coba Lagi</button>
    </div>
  );
    ...
    <div className="p-6 max-w-7xl mx-auto min-h-screen bg-gray-50/50">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 gap-4">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-1">Kelola proyek perumahan dan unit rumah</p>
        </div>
        <button
      ...
      {propertiList.length === 0 ? (
        <div className="text-center py-20 bg-white rounded-2xl border border-dashed border-gray-300">
          ...
          <p className="text-gray-500 font-medium">Belum ada properti yang ditambahkan.</p>
          <button onClick={openAddProperti} className="mt-2 text-blue-600 font-medium hover:underline">
            Tambah sekarang
          </button>
        </div>
      ) : (
              ...
              <div className="flex flex-col lg:flex-row justify-between gap-6 pb-6 border-b border-gray-100">
                <div className="flex-1">
                  <div className="flex items-start justify-between">
                    ...
                    {/* Action Buttons (Mobile View handled by layout) */}
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-5 ml-0 lg:ml-8">
                    ...
                    <div className="flex items-center gap-3 text-sm text-gray-600 bg-gray-50 p-3 rounded-lg">
                      ...
                      <span className="truncate">{p.kontak_kontraktor || "-"}</span>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="pt-6">
                ...
                {!p.rumahs || p.rumahs.length === 0 ? (
                  <div className="text-center py-8 bg-gray-50 rounded-xl border border-dashed border-gray-200">
                    <p className="text-sm text-gray-400">Belum ada unit rumah di properti ini.</p>
                  </div>
                ) : (
                      ...
                      return (
                        ...
                        >
                           ...
                           <div className={`absolute top-0 right-0 px-3 py-1 rounded-bl-xl text-[10px] font-bold uppercase tracking-wider ${
                             isSold ? "bg-gray-200 text-gray-600" : "bg-green-100 text-green-700"
                           }`}>
                          ...
                          <div className="flex justify-between items-start mt-2">
                            <div>
                              <p className="font-bold text-gray-800 text-lg">Tipe {r.tipe}</p>
      ...
      {isRumahModalOpen && (
        <RumahFormModal
          // FIX: Gunakan 'editData' (sesuai modal yang sudah diperbaiki sebelumnya)
          editData={editingRumah}
          // Opsional: Kirim ID properti jika mode tambah baru (agar dropdown otomatis terisi)
          // Anda mungkin perlu menyesuaikan RumahFormModal untuk menerima prop ini jika ingin otomatis
